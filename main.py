...
#子弹类
#pygame.sprite.Sprite是一个图片实体对象，可以是怪，也可以是人物，或者子弹

class Bullet(pygame.sprite.Sprite):
    def __init__(self,bullect_img,init_pos):
        pygame.sprite.Sprite.__init__(self)
        self.image=bullect_img
...
class Player(pygame.sprite.Sprite):
    def __init__(self,plane_img,player_rect,init_pos):
        ...
        self.rect=player_rect[0]  #初始化图片所在举行
        self.rect.topleft=init_pos #初始化举行的左上角坐标
...
def startGame():
    ...
    # 临时的变量在到排行榜的时候使用
    j = 0
    #获取文件中内容转换成列表使用rah分割开内容
    arrayscore = read_txt(r'score.txt')[0].split('rah')
    # 循环分数列表在列表里排序
    ...
    for i in range(0, len(arrayscore)):
        ...
        if i == 0:
            # 覆盖写入内容追加rah方便分割内容
            write_txt(arrayscore[i] + 'rah', 'w', r'score.txt')
        else:
            # 判断是否为最后一个分数
            if (i == 9):
                # 最近添加内容最后一个分数不添加rah
                ...
                # 不是最后一个分数，添加的时候添加rah
                write_txt(arrayscore[i] + 'rah', 'a', r'score.txt')
